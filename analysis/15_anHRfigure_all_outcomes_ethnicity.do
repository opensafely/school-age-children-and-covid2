...
_tab ("ethnicity") _tab ("outcome") _tab ("HR")  _tab ("lci")  _tab ("uci")  _n
foreach dataset in W2 {
forvalues x=0/1 {
...
local endwith "_tab"
	...
	*put the varname and condition to left so that alignment can be checked vs shell
	file write tablecontents_all_outcomes ("`dataset'") _tab ("`x'") _tab ("`variable'") _tab ("`i'") _tab ("`eth'") _tab ("`outcome'") _tab
		...
		*3) Save the estimates for plotting
		if `noestimatesflag'==0{
			if "`modeltype'"=="plus_ethadj"   {
				...
				local ub = r(ub)
				local N=e(N)
				post HRestimates_all_outcomes ("`dataset'") ("`x'") ("`outcome'") ("`variable'") (`i') (`ethnicity') (`hr') (`lb') (`ub') (`N') 
				}
		}	
		} /*min adj, full adj*/
	} /*datasets*/
...
cap postutil clear
postfile HRestimates_all_outcomes str10 dataset str10 x str10 ethnicity str10 outcome str27 variable i hr lci uci N using `HRestimates_all_outcomes'
...
foreach dataset in W2 { 
foreach age in 0 1 {
foreach ethnicity int_level in 1 2 3 4 5 {
use `HRestimates_all_outcomes', clear
...
graph export ./output/15_an_HRforest_all_outcomes_ageband_`age'_`dataset'_ethnicity`int_level'.svg, as(svg) replace
}
}
}
